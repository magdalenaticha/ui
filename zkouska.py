...
# === Načtení reálných genových dat ===
with h5py.File("real_genes.h5", "r") as f:
    data = f["gene_expression"][:]  # očekává dataset s hodnotami genů
    genes = [g.decode("utf-8") for g in f["gene_expression"].attrs["columns"]]
    samples = [s.decode("utf-8") for s in f["gene_expression"].attrs["index"]]

expression_df = pd.DataFrame(data, index=samples, columns=genes)
expression_df["patient_id"] = expression_df.index

# vytvoření základních klinických dat, aby appka fungovala dál
clinical_df = pd.DataFrame({
    "patient_id": samples,
    "age": np.random.normal(65, 10, len(samples)).astype(int),
    "gender": np.random.choice(["Male", "Female"], len(samples)),
    "survival_time": np.random.randint(100, 1000, len(samples)),
    "event_occurred": np.random.choice([0, 1], len(samples), p=[0.3, 0.7]),
    "treatment": np.random.choice(["Drug_A", "Drug_B", "Placebo"], len(samples))
})
